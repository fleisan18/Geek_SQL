...
CREATE OR REPLACE VIEW users_less_20 AS (
SELECT CONCAT(firstname, ' ', lastname) AS user, hometown, gender
FROM users JOIN profiles ON users.id=profiles.user_id
WHERE TIMESTAMPDIFF(YEAR, birthday, curdate()) <= 20);

SELECT * FROM users_less_20;
...
SELECT DISTINCT user, cnt_messages,
DENSE_RANK() OVER (ORDER BY cnt_messages DESC) AS dence_rank
FROM
	(SELECT CONCAT(firstname, ' ', lastname) AS user,
	COUNT(messages.id) OVER w AS cnt_messages
	FROM users JOIN messages ON users.id = messages.from_user_id
	WINDOW w AS (PARTITION BY from_user_id)) AS sub1;
...
TIMESTAMPDIFF(MINUTE, created_at, LEAD(created_at) OVER (ORDER BY created_at)) AS minute_diff
FROM messages
ORDER BY created_at;