...
SELECT from_user_id, COUNT(id) AS cnt_messages
FROM messages
...
HAVING cnt_messages = (
	SELECT COUNT(id) AS cnt_messages
	FROM messages
	WHERE from_user_id IN
		(SELECT initiator_user_id AS 'id' FROM friend_requests
		WHERE target_user_id = 1 AND status = 'approved'
