...
-- Задача 1. Создайте процедуру, которая выберет для одного пользователя 5 пользователей в случайной
-- комбинации, которые удовлетворяют хотя бы одному критерию:
-- 1) из одного города
-- 2) состоят в одной группе
-- 3) друзья друзей
DROP PROCEDURE IF EXISTS random_friend;

DELIMITER //
CREATE PROCEDURE random_friend (user_id INT)
BEGIN
WITH friends AS (
	...
	SELECT p1.user_id FROM profiles p1 JOIN profiles p2 ON p1.hometown = p2.hometown
    WHERE p2.user_id = user_id AND p1.user_id != user_id
	UNION
    SELECT uc1.user_id FROM users_communities uc1 JOIN users_communities uc2 ON uc1.community_id = uc2.community_id
    WHERE uc2.user_id = user_id AND uc1.user_id != user_id
	UNION
		SELECT fr.initiator_user_id AS friend_id
		FROM friends f
		JOIN friend_requests fr ON fr.target_user_id = f.id
		WHERE fr.initiator_user_id != user_id  AND fr.status = 'approved'
	UNION
...
DELIMITER ;
CALL random_friend(1);

-- Задача 2. Создание функции, вычисляющей коэффициент популярности пользователя
-- (по заявкам на дружбу – таблица friend_requests)
DROP FUNCTION IF EXISTS popularity_koef;
DELIMITER //
CREATE FUNCTION popularity_koef (user_id INT)
RETURNS FLOAT
READS SQL DATA
BEGIN
	DECLARE cnt_sent INT;
	DECLARE cnt_received INT;

	SET cnt_sent = (SELECT COUNT(*) AS cnt_sent
	FROM friend_requests fr
    WHERE initiator_user_id = user_id);
    
    SELECT COUNT(*) INTO cnt_received
	FROM friend_requests fr
    WHERE target_user_id = user_id;
    /*SET cnt_received = (
    SELECT COUNT(*) AS cnt_received
	FROM friend_requests fr
    WHERE target_user_id = user_id);*/
	RETURN cnt_received/cnt_sent;
END //
DELIMITER ;

SELECT popularity_koef(1);
