...
AND media_type_id IN (SELECT id FROM media_types
...
SELECT users.id, CONCAT(firstname, ' ', lastname) AS 'Пользователь', hometown, filename
...
SELECT media.id, filename, name_type
FROM media JOIN media_types ON media.media_type_id = media_types.id
ORDER BY media.id;
